...
基于 [PuTTY](https://www.chiark.greenend.org.uk/~sgtatham/putty/) 的 Windows SSH 客户端，附带一个按需使用的 AI 助手。它帮助用户理解终端输出、解释命令、整理排查思路，并提供可供人工检查的候选命令。
...
> [!IMPORTANT]
> `v1.0.6` 直接按当前显示器的工作区设置首次窗口大小（68% 宽、62% 高），不再受已保存会话中 80×24 终端网格的限制。请把它当作一个按需问答的 AI 助手：它只在用户提问后返回解释、排查建议或候选命令，不会自主规划任务、持续操作或替用户做决定。AI 输出仍可能出错，执行任何生成命令前必须人工复核，不建议用于无人值守生产操作。

本项目由独立开发者维护，不隶属于 PuTTY、OpenAI、任何模型服务商或堡垒机产品供应商，也不代表这些项目或机构获得授权、赞助或认可。第三方名称仅用于说明兼容性和许可证归属，相关权利归各自权利人所有。

## 这是一个什么工具

PuTTY-Assistant 是一个放在 SSH 终端旁边的 AI 助手。它不接管终端，也不替用户完成任务。用户提出问题后，可以选择是否附带经过处理的终端上下文；模型返回文字解释、排查步骤或候选命令，用户再决定是否回填和执行。

## 明确边界

- 它不是副驾驶，不会主动观察并推进任务。
- 它不是 agent 或智能体，不会自主调用工具、连续执行命令或管理远程主机。
- 它不会自动按回车执行生成的命令，高风险操作仍由用户确认。
...
它适合用来解释报错、阅读日志、学习命令、整理排查步骤，以及把已经理解的候选命令回填到终端。
...
仓库中的 Windows `putty` 目标会生成带原生 AI 助手面板的 `putty.exe`。实现仅依赖 Windows 自带的 WinHTTP、Rich Edit 和数据保护 API，不需要浏览器组件。
...
建立 SSH 会话后，右侧会显示 PuTTY-Assistant AI 助手面板：
...
- 程序默认记录不含问题、回复、上下文、命令正文和 API Key 的元数据审计日志，写入当前用户本地应用数据目录下的 `audit.log`。日志只包含时间、事件类型、模型端点主机和风险级别等信息。
...
Windows 发布包位于 `package/` 目录，包含 `putty.exe`、应用本地 VC Runtime、项目与 PuTTY 许可证、第三方说明和发布说明。
...
欢迎通过 Issue 提交使用场景、功能建议和问题反馈，也欢迎参与 AI 助手面板、模型接入、安全策略与文档等方向的开发。
