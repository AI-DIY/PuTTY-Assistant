...
![Version](https://img.shields.io/badge/version-v1.0.5-success?style=flat-square)

</div>

> [!IMPORTANT]
> `v1.0.5` 优化了首次启动时的工作区比例：默认终端从 80×24 调整为 104×36，使终端与 AI 侧边栏拥有更均衡的初始空间。Windows 客户端已集成 AI 侧边栏、可选终端上下文、兼容模型接口、命令确认与安全控制。AI 输出仍可能出错，执行任何生成命令前必须人工复核，暂不建议直接用于无人值守生产操作。
...
打包产物位于 `package/PuTTY-AI-v1.0.5-windows-x64.zip`，包含 `putty.exe`、应用本地 VC Runtime、项目与 PuTTY 许可证、第三方说明和发布说明。
